...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, n_agent, fc1_units=128, fc2_units=128):
        ...
        # Toggle penalties ON / OFF
        self.USE_SMOOTHNESS = False
        self.USE_SYMMETRY = True
        self.USE_MAGNITUDE = False

        # Penalty weights (tune independently)
    ...
    def save_step(self, trajectory):
        """
        Save each step to current trajectory, with optional reward shaping.
        trajectory = [states, values, actions, log_probs, rewards, terminals]
        """
        states, values, actions, log_probs, rewards, terminals = trajectory

        actions = np.array(actions)
        rewards_env = np.array(rewards)

        # Initialize previous actions
        if self.prev_actions is None:
            self.prev_actions = np.zeros_like(actions)

        rewards_new = rewards_env.copy()

        # ================= Smoothness penalty =================
        if self.USE_SMOOTHNESS:
            diff = actions - self.prev_actions
            smooth_pen = np.sum(diff ** 2, axis=1)
            rewards_new -= self.LAMBDA_SMOOTH * smooth_pen
        ...
        if self.USE_SYMMETRY:
            ...
            left = actions[:, :half]
            right = actions[:, half:]
            sym_pen = np.sum((left - right) ** 2, axis=1)
            rewards_new -= self.LAMBDA_SYM * sym_pen

        # ================= Magnitude penalty =================
        if self.USE_MAGNITUDE:
            mag_pen = np.sum(actions ** 2, axis=1)
            rewards_new -= self.LAMBDA_MAG * mag_pen

        # Update previous actions
        self.prev_actions = actions.copy()

        # Store trajectory with SHAPED rewards
        self.trajectory.append(
            [states, values, actions, log_probs, rewards_new, terminals]
        )
